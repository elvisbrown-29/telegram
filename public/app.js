...
const botAPI = {
    ...
    async getReferralLink(userId) {
        try {
            const response = await fetch(`${API_ENDPOINT}/bot/user/${userId}/referral-link`, {
                headers: {
                    'Authorization': `Bot ${botToken}`,
                    'Content-Type': 'application/json'
                }
...
const updateUI = {
    ...
    price(priceData) {
        ...
        console.log('Updating price UI with:', priceData);

        // Update price with animation
        priceElement.textContent = formatUSD(priceData.price);
        changeElement.textContent = formatPercent(priceData.change24h || 0);
        changeElement.className = `price-change ${(priceData.change24h || 0) >= 0 ? 'positive' : 'negative'}`;
    },
    ...
    referrals(referralData) {
        ...
        elements.totalReferrals.textContent = referralData.total || '0';
        elements.referralEarnings.textContent = formatTON(referralData.earnings || 0);
        
        // Update referral link
        if (elements.referralLink && referralData.link) {
            elements.referralLink.value = referralData.link;
        }

        // Update referral history
        if (referralData.history && referralData.history.length > 0) {
            elements.referralHistory.innerHTML = referralData.history
                .map(ref => `
                    <div class="referral-item">
                        <img src="${ref.avatar || 'assets/default-avatar.png'}" alt="Avatar" class="referral-avatar">
                        <div class="referral-info">
                            <div class="referral-name">${ref.name || 'Anonymous'}</div>
                            <div class="referral-date">
                                <span class="material-icons">schedule</span>
                                ${formatDate(ref.date)}
                            </div>
                        </div>
                        <div class="referral-amount">+${formatTON(ref.earnings)} TON</div>
                    </div>
                `).join('');
        } else {
            elements.referralHistory.innerHTML = `
...
// Handle referral link copying and sharing
const setupReferralActions = () => {
    ...
    if (elements.shareLinkBtn) {
        elements.shareLinkBtn.addEventListener('click', () => {
            webapp.switchInlineQuery(elements.referralLink.value, ['users', 'groups']);
        });
    }
};
...
const init = async () => {
    try {
        ...
        if (!telegramUser) {
            console.log('No Telegram user data available, running in standalone mode');
            return;
        }
        ...
        // Start real-time updates for user data
        setInterval(updateUserProfile, REFRESH_INTERVAL);
